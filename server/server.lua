...
CreateThread(function()
    while true do
        Wait(500)--Change to send every minute in game sync
        TriggerClientEvent('qb-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
    end
end)
...
AddEventHandler('txAdmin:events:scheduledRestart', function(eventData) -- Tsunami Weather Fuckery
    if eventData.secondsRemaining == 1800 then -- 30 Minutes Remaining
        print("30 minutes before restart... Gradually changing weather to thunder!")
        setDynamicWeather(false)
        setWeather("CLOUDS")
        TriggerEvent('qb-weathersync:server:RequestStateSync')
    elseif eventData.secondsRemaining == 1500 then -- 25 Minutes Remaining
        setWeather("RAIN")
        TriggerEvent('qb-weathersync:server:RequestStateSync')
    elseif eventData.secondsRemaining == 600 then -- 10 Minutes Remaining
        setWeather("THUNDER")
        TriggerEvent('qb-weathersync:server:RequestStateSync')
    elseif eventData.secondsRemaining == 180 then -- 3 Minute Remaining
        setBlackout(true)
        setWeather("THUNDER")
        TriggerEvent('qb-weathersync:server:RequestStateSync')
